...
| `gpt-6-sol` | $2.00 | $0.20 | 未配置 | $10.00 | 2026-09-23 按维护者指定基础价格新增 |
| `gpt-6-luna` | $0.10 | $0.01 | 未配置 | $0.50 | 2026-09-23 按维护者指定基础价格新增 |
...
## 2026-09-23：新增 GPT-6 Sol / Luna

按维护者提供的价格，为 `gpt-6-sol` 和 `gpt-6-luna` 新增独立定价对象，
同时更新 SHA-256。价格顺序为输入 / 输出 / 缓存读取，单位为美元/百万 Token：

- `gpt-6-sol`：`$2 / $10 / $0.20`。
- `gpt-6-luna`：`$0.10 / $0.50 / $0.01`。

仅配置上述三项基础价格及解析所需的模型元数据。维护者未指定缓存写入、
Priority/Flex/Batch、长上下文价格或上下文上限，因此未添加这些字段；
当前 Sub2API 对缺失的缓存写入价格按 `0` 解析。

运行中的 Sub2API `0.2.4-rebuilt-imgdiag` 在这两个模型缺少目录条目时，会将基础价格
回退到 `gpt-5.4`。渠道定价可覆盖客户计费，但模型文件仍用于默认计价及部分账号成本统计。
新增精确模型条目可避免这两种模型继续使用其他模型的兜底价格。
